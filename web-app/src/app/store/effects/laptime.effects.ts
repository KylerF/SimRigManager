...
export class LaptimeEffects {
    ...
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.lapTimeGQL.fetch(action.params).pipe(
          ...
          catchError((error) =>
            of(
              laptimeActions.LoadLaptimesFailure({
                payload: {
                  error: error.message,
                },
              })
  ...
  streamLaptimes$ = createEffect(() =>
    this.actions$.pipe(
      ofType(laptimeActions.StreamLaptimes),
      mergeMap(() =>
        this.lapTimeService.streamLapTimes().pipe(
          takeUntil(this.actions$.pipe(ofType(laptimeActions.StopStreamLaptimes))),
          map((laptime) =>
            laptimeActions.AddLaptime({
              payload: {
                data: laptime,
              },
            })
