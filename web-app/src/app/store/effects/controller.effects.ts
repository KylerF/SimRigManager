...
export class ControllerEffects {
  ...
  loadControllers$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.controllerService.getControllers().pipe(
          map((controllers) =>
            controllerActions.LoadControllersSuccess({
              payload: { data: controllers },
            })
          ),
          catchError((error) =>
            of(
              controllerActions.LoadControllersFailure({
   ...
   */
  updateControllerState$ = createEffect(() =>
    this.actions$.pipe(
      ofType(controllerActions.UpdateControllerState),
      mergeMap((action) =>
        this.controllerService.getControllerState(action.controller).pipe(
          map((controllerState) =>
            controllerActions.UpdateControllerStateSuccess({
              payload: {
   ...
   */
  streamControllerState$ = createEffect(() =>
    this.actions$.pipe(
      ofType(controllerActions.StartStream),
      mergeMap((action) =>
        this.controllerService.getStateStream(action.controller).pipe(
          map((controllerState) =>
            controllerActions.UpdateControllerStateSuccess({
              payload: {
                controller: action.controller,
   ...
   */
  getControllerSettings$ = createEffect(() =>
    this.actions$.pipe(
      ofType(controllerActions.GetControllerSettings),
      switchMap((action) =>
        this.controllerService
          .getControllerSettings(action.payload.data.controller, action.payload.data.driver)
          .pipe(
            map((controllerSettings) =>
              controllerActions.GetControllerSettingsSuccess({
                payload: {
            ...
            catchError((error) =>
              of(
                controllerActions.GetControllerSettingsFailure({
                  payload: error,
                })
              )
            )
    ...
    this.actions$.pipe(
      ...
      switchMap((action) => {
        return this.controllerService.addController(action.payload.data).pipe(
          ...
          catchError((error) => {
            return of(
              controllerActions.CreateControllerFailure({
                payload: { error: error },
              })
            );
          })
   ...
   */
  updateControllerSettings$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) => {
        return this.controllerService.updateController(action.controller).pipe(
          ...
          catchError((error) => {
            return of(
              controllerActions.UpdateControllerSettingsFailure({
                payload: { error: error },
              })
            );
          })
    ...
    this.actions$.pipe(
      ...
      switchMap((action) => {
        return this.controllerService.deleteController(action.payload.data).pipe(
          ...
          catchError((error) => {
            return of(
              controllerActions.DeleteControllerFailure({
                payload: { error: error },
              })
            );
          })
