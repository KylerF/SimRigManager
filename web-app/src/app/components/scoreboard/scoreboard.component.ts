...
import { State, selectLaptimesState, selectAllLaptimes } from 'store/reducers';

import {
  LoadLaptimes,
  StreamLaptimes,
  StopStreamLaptimes,
} from 'src/app/store/actions/laptime.actions';
...
export class ScoreboardComponent implements OnInit, OnDestroy {
  ...
  constructor(private driverService: DriverService, private store: Store<State>) {}
  ...
  getLapTimes() {
    // Get all lap times
    this.store.dispatch(LoadLaptimes(this.filterParams));

    // And subscribe to new ones
    this.store.dispatch(StreamLaptimes());

    this.laptimesState$ = this.store.pipe(select(selectLaptimesState()));
    this.filteredLaptimes$ = this.store.pipe(select(selectAllLaptimes()));
  }
  ...
  showOverallBestTimes() {
    this.filteredLapTimes = this.lapTimes.filter(
      (lapTime) =>
        lapTime.time ==
        Math.min(
          ...this.lapTimes
  ...
  showDriverBestTimes() {
    this.driverService.selectedDriver$.subscribe((activeDriver) => {
      this.filteredLapTimes = this.lapTimes.filter(
        (lapTime) => lapTime.driver.id == activeDriver.id
      );
    });
  }
    ...
    if (this.searchText.length > 0) {
      if (this.searchColumn == 'driver') {
        this.filteredLapTimes = this.filteredLapTimes.filter((lapTime) =>
          lapTime.driver.name
            .toLocaleLowerCase()
            .includes(this.searchText.trim().toLocaleLowerCase())
        );
      } else {
        this.filteredLapTimes = this.filteredLapTimes.filter((lapTime) =>
          lapTime[this.searchColumn]
            .toLocaleLowerCase()
            .includes(this.searchText.trim().toLocaleLowerCase())
        );
      }
  ...
  sortScores(column: string, order: string = 'asc') {
    ...
    this.filteredLapTimes.sort((lapTime1, lapTime2) => {
      if (column == 'driver') {
        ...
        return order == 'desc'
          ? lapTime2[column] > lapTime1[column]
            ? 1
            : lapTime2[column] < lapTime1[column]
            ? -1
            : 0
          : lapTime1[column] > lapTime2[column]
          ? 1
          : lapTime1[column] < lapTime2[column]
          ? -1
          : 0;
